...
import { FileBox, Search, ChevronRight, Hash, User, Calendar, Clock } from "lucide-react";
...
const AttendanceRecap = () => {
  ...
  const fetchRecords = async () => {
    setLoading(true);
    try {
      const collName = activeTab === "staff" ? "staff_attendance" : "student_attendance";
                ...
                <div className="flex flex-wrap items-center gap-3 md:gap-8">
                  <div className="flex flex-col items-end">
                    ...
                    <div className="flex items-center gap-1.5 text-primary font-black text-xs italic">
                      <Clock size={14} />
                      {record.timestamp?.toDate ? format(record.timestamp.toDate(), "HH:mm") : "..."}
                    </div>
                  </div>
